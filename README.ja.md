...
- **ボタンの壁**：グループごとに並んだ音声ボタン。クリックで再生、連続再生にも対応。各ボタン内のⓘ情報コントロールは右側に固定され、専用の幅と等しい内側余白を確保するため、ホバーしてもレイアウトが動かず、詳細を開いた後もポインターを外せば情報コントロールが閉じます；
...
- **投稿項目を明確化**：ソース情報は常に表示し、必須項目にはアスタリスクを付け、任意項目には付けません；
...
| `BILI_ACCESS_KEY_ID`<br/>`BILI_ACCESS_KEY_SECRET` | 開発者キーペア（🔒 機密） | 公式審査の承認後、公式からメールで送付 |
| `BILI_ROOM_OWNER_AUTH_CODE` | 配信者の身分コード・身份码（🔒 機密） | [play-live.bilibili.com](https://play-live.bilibili.com/)（配信者向け「幻星」ポータル） |

申請の手順：

1. Bilibili アカウントで [open-live.bilibili.com](https://open-live.bilibili.com/) にログインし、**クリエイターサービスセンター**へ。案内に従って本人確認を済ませ、**個人開発者**として入居申請を提出（企業資格は不要）；
2. 公式審査の承認後、`access_key_id` / `access_key_secret` は公式からメールで送付されます；
