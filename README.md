...
## 📦 最新发布：v2.7.0（2026-08-10）

本次 release 完成 INIT-002 的前端与后台改造，并收口验收阶段发现的交互细节：

- **后台本地免登录验收**：开发环境会注入本机管理员会话，打开 `/admin` 即可进入后台；生产环境不会启用这条路径；
- **按钮信息交互稳定**：ⓘ 固定在按钮内部右侧，预留稳定宽度与等距内边距，悬浮不会让按钮跳动，点击后移开鼠标也会正确收起；
- **投稿字段更清楚**：来源信息始终可见，必填字段使用星号标记，选填字段不再使用“可不填”提示；
- **质量门补齐**：前端 UI 合约测试、服务端测试、后台配色对比度检查与构建验证均纳入本次验收；
- **仓库清理**：历史 `docs/` 已从版本库移除，并由 `.gitignore` 继续忽略本地同名目录。

完整发布说明：[v2.7.0 release](https://github.com/ryanlan-new/joi-button/releases/tag/v2.7.0)。

## ✨ 功能全览

### 🎧 访客这一侧

- **按钮墙**：按分组陈列的音声按钮，点击即播，支持连播；
