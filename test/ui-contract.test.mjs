...
test('INC-010/011: the badge floats beyond the pill without reserving hidden space', () => {
  assert.match(homeSource, /\.clip-info-badge[\s\S]*?left:\s*calc\(100%\s*\+\s*\.5rem\)/)
  assert.doesNotMatch(homeSource, /\.voice-play[\s\S]*?padding-right:\s*30px/)
